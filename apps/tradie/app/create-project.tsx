...
import { supabase, useUser, cn, TRADE_CATEGORIES } from '@tradie/core';
                ...
                <ScrollView className="flex-1 px-6 pt-8" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
                                ...
                                {TRADE_CATEGORIES.map((item) => (
                                    <TouchableOpacity
                                        ...
                                        className={cn(
                                            ...
                                            type === item.id ? "border-primary bg-primary/5" : "border-slate-100"
                                        )}
                                    >
                                        <View className={cn("w-11 h-11 rounded-full items-center justify-center mb-2.5", type === item.id ? "bg-primary" : "bg-slate-100")}>
                                            {/* @ts-ignore */}
                                            <item.icon size={22} color={type === item.id ? "white" : "#64748B"} />
                                        </View>
                                        <Typography variant="caption" className={cn("text-xs font-semibold text-center", type === item.id ? "text-primary" : "text-slate-600")}>{item.id}</Typography>
                                        {type === item.id && (
                    ...
                    {step === 2 && (
                        <View>
                            ...
                            <Input placeholder={TITLE_PLACEHOLDERS[type] || 'e.g. General home maintenance'} value={title} onChangeText={setTitle} className="text-lg h-16" />
                        </View>
                    )}
                    ...
                    {step === 3 && (
                        <View>
                            ...
                            <Input placeholder="e.g. 123 Smith St, Collingwood VIC" value={address} onChangeText={setAddress} className="text-lg h-16" />
                        </View>
                    )}
                    ...
                    {step === 4 && (
                        <View>
                            ...
                            <Typography variant="body" className="text-slate-500 mb-8">The more detail you provide, the better quotes you'll get.</Typography>
                            <Input placeholder="Describe the scope of work..." value={description} onChangeText={setDescription} multiline numberOfLines={6} style={{ height: 180, textAlignVertical: 'top' }} />
                    ...
                    {step === 5 && (
                        <View>
                            ...
                            <Typography variant="body" className="text-slate-500 mb-8">This helps tradies decide if the job is right for them.</Typography>
                            <View className="gap-3">
                                {BUDGET_RANGES.map((range) => (
                                    <TouchableOpacity key={range.id} onPress={() => setBudgetRange(range.id)}
                                        className={cn("w-full py-4 px-5 rounded-2xl border flex-row items-center justify-between", budgetRange === range.id ? "bg-primary border-primary" : "bg-white border-slate-100")}>
                                        <Typography variant="body" className={cn("font-semibold text-base", budgetRange === range.id ? "text-white" : "text-slate-700")}>{range.label}</Typography>
                                        {budgetRange === range.id && (<View className="bg-white/20 rounded-full p-1"><CheckIcon size={14} color="white" /></View>)}
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
                        ...
                        <View>
                            ...
                            <View className="gap-3">
                                {URGENCY_OPTIONS.map((option) => (
                                    <TouchableOpacity key={option.id} onPress={() => setUrgency(option.id)}
                                        className={cn("flex-row items-center p-5 rounded-2xl border", urgency === option.id ? "bg-primary/5 border-primary" : "bg-white border-slate-100")}>
                                        <View className={cn("w-12 h-12 rounded-full items-center justify-center mr-4", urgency === option.id ? "bg-primary" : "bg-slate-100")}>
                                            {/* @ts-ignore */}
                                            <option.icon size={24} color={urgency === option.id ? "white" : "#64748B"} />
                                        </View>
                                        <View className="flex-1">
                                            <Typography variant="h3" className={cn("text-base mb-0.5", urgency === option.id ? "text-primary" : "text-slate-900")}>{option.label}</Typography>
                                            <Typography variant="caption" className="text-slate-500 text-xs">{option.description}</Typography>
                                        </View>
                                        {urgency === option.id && (<View className="bg-primary rounded-full p-1 ml-2"><CheckIcon size={14} color="white" /></View>)}
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
                                    ...
                                    <View className="flex-row justify-between items-start mb-3">
                                        <View className="flex-row items-center flex-1">
                                            <View className="w-11 h-11 rounded-full bg-primary items-center justify-center mr-3">
                                                {(() => { const SelectedIcon = TRADE_CATEGORIES.find(t => t.id === type)?.icon || WrenchScrewdriverIcon; return <SelectedIcon size={22} color="white" />; })()}
                                            </View>
                                            <View className="flex-1">
                                        ...
                                        <TouchableOpacity onPress={() => setStep(1)} className="p-1.5"><PencilSquareIcon size={18} color="#94A3B8" /></TouchableOpacity>
                                    </View>
                                    {description ? <Typography variant="body" className="text-slate-500 text-sm leading-5">{description}</Typography> : null}
                                </View>
                                <ReviewRow icon={<MapPinIcon size={18} color="#64748b" />} label="Location" value={address} onEdit={() => setStep(3)} />
                                <ReviewRow icon={<CurrencyDollarIcon size={18} color="#64748b" />} label="Budget" value={BUDGET_RANGES.find(b => b.id === budgetRange)?.label || '—'} onEdit={() => setStep(5)} />
                                <ReviewRow icon={<ClockIcon size={18} color="#64748b" />} label="Timeline" value={URGENCY_OPTIONS.find(u => u.id === urgency)?.label || '—'} onEdit={() => setStep(6)} isLast />
                            </View>
                        </View>
                    )}
                </ScrollView>
                <View className="p-6 bg-white border-t border-slate-100" style={{ paddingBottom: insets.bottom + 20 }}>
                    <Button label={step === TOTAL_STEPS ? "Post Project" : "Continue"} onPress={handleNext} loading={loading} size="lg" />
                </View>
...
function ReviewRow({ icon, label, value, onEdit, isLast = false }: { icon: React.ReactNode; label: string; value: string; onEdit: () => void; isLast?: boolean; }) {
    return (
        <View className={cn("flex-row items-center px-5 py-4", !isLast && "border-b border-slate-50")}>
            <View className="w-9 h-9 rounded-xl bg-slate-50 items-center justify-center mr-3">{icon}</View>
            <View className="flex-1">
            ...
            <TouchableOpacity onPress={onEdit} className="p-1.5"><PencilSquareIcon size={16} color="#94A3B8" /></TouchableOpacity>
        </View>
    );
}
