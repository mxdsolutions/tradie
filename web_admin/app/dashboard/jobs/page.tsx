...
export default function JobsPage() {
    ...
    const filteredJobs = jobs.filter(job =>
        ...
        job.sku.toLowerCase().includes(search.toLowerCase())
    );
        ...
        <DashboardPage>
            ...
            <DashboardControls>
                <div className="relative flex-1 max-w-sm">
                    ...
                    <Input
                        placeholder="Search jobs or reference..."
                        ...
                        {filteredJobs.map((job) => (
                            <tr key={job.id} className={tableRow + " group cursor-pointer"}>
                                ...
                                <td className={tableCell + " px-4 text-right sm:text-left"}>
                                    <span className="font-bold text-sm">{job.price}</span>
                                </td>
                                <td className={tableCell + " px-4 hidden sm:table-cell"}>
                                    <div className="flex items-center gap-2">
                                        <div className={`w-1.5 h-1.5 rounded-full ${job.status === "Active" ? "bg-emerald-500" : "bg-amber-500"}`} />
